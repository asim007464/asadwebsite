...
import { createCategory, deleteCategory, updateCategory, updateCategoryAppearance } from "@/app/admin/actions";
...
function errMsg(code: string) {
  ...
  if (code === "slug") return "Enter a valid slug (letters, numbers, hyphens) or leave blank to derive from the name.";
  if (code === "id") return "Missing category id.";
  return code.length < 260 ? code : "Something went wrong.";
}

function noticeMsg(code: string) {
  if (code === "category-saved") return "Category name and URL slug were saved.";
  return "";
}
  ...
  return (
    <main className="py-6 lg:py-0">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-600">
              <span className="font-semibold text-slate-800">Add</span> new categories,{" "}
              <span className="font-semibold text-slate-800">edit</span> their name, URL slug, and optional parent,{" "}
              <span className="font-semibold text-slate-800">delete</span> ones you no longer need (products keep working — their category is cleared). Thumbnails and{" "}
              <span className="font-semibold">hero icon hints</span> are for the storefront carousel and grids.
            </p>
            <p className="mt-2 text-xs text-slate-500">
              Slugs must stay unique — they appear in URLs like <code className="rounded bg-slate-100 px-1">/products?category=your-slug</code>.
            </p>
          </div>
        ...
        {notice ? (
          <div className="mt-5 rounded-2xl border border-emerald-200/80 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-950">{notice}</div>
        ) : null}
        {error ? (
          <div className="mt-5 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-800">{error}</div>
        ) : null}

        <form action={createCategory} className="mt-6 grid grid-cols-1 gap-3 rounded-3xl border border-dashed border-blue-200 bg-blue-50/40 p-4 sm:grid-cols-12 sm:p-5">
                ...
                <div className="flex flex-wrap items-start gap-6">
                  ...
                  <div className="grid min-w-0 flex-1 gap-6 lg:grid-cols-2">
                    <div className="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm">
                      <div className="text-xs font-bold uppercase tracking-wide text-slate-700">Details — name &amp; URL</div>
                      <form action={updateCategory} className="mt-4 space-y-3">
                        ...
                        <div>
                          <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">Parent category (optional)</label>
                          <select name="parent_id" defaultValue={c.parent_id ?? ""} className={inputClass}>
                    ...
                    <form action={updateCategoryAppearance} className="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm">
                      <input type="hidden" name="id" value={c.id} />
                      <div className="text-xs font-bold uppercase tracking-wide text-slate-700">Storefront visuals</div>
